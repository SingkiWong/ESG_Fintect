...
def baidu(company):
	...
	for k_name in company_1:
		...
		file_write_obj.write(k_name+str(a))
		file_write_obj.write('\n')
		for i in range(0,7):
			num = i * 10					
			# 1.获取网页源代码（参考2.3、3.1、3.4节）			
			url='https://www.baidu.com/s?tn=news&rtt=1&bsst=1&cl=2&wd=%s&pn=%s'%(k_name,num) #rtt=1是焦点
			...
			date = []
			# title=[title[i].strip() for i in range(len(title))]
			# title=[re.sub('<.*?>', '', title[i]) for i in range(len(title))]
			# info=[re.sub('<.*?>', '', title[i]) for i in range(len(title))]
			# source=[info_k.split('&nbsp;&nbsp;')[0] for info_k in info]
			# source=[info_k.split('&nbsp;&nbsp;')[1] for info_k in info]
			# source=[source[i].strip() for i in range(len(title))]
			# date=[date[i].strip() for i in range(len(title))]
			# date=[date[i].split(' ')[0] for i in range(len(title))]
			# date=[re.sub('年', '-', date[i]) for i in range(len(title))]
			# date=[re.sub('月', '-', date[i]) for i in range(len(title))]
			# date=[re.sub('日', '-', date[i]) for i in range(len(title))]
			# date=[time.strftime("%Y-%m-%d") if ('小时' in date[i]) or ('分钟' in date[i]) else date[i] for  i in range(len(title))]
...
for company in companies:
	...
	try:
		...
		a=time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))	
		file_write_obj.write(company + '数据爬取并存入数据库成功'+" "+str(a))
		file_write_obj.write('\n')
		file_write_obj.close()
	except:
		print(company + '数据爬取并存入数据库失败')
		a=time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))
		file_write_obj.write(company + '数据爬取并存入数据库失败'+" "+str(a))
		file_write_obj.write('\n')
		file_write_obj.close()
